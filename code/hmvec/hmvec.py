...
class HaloModel(Cosmology):
    ...
    def deltav(self,z): 
        # Duffy virial actually uses this from Bryan and Norman 1997
        # return 178. * self.omz(z)**(0.45) # Eke et al 1998
        # omz = omega matter at redshift z
        x = self.omz(z) - 1.
    ...
    def rvir(self,m,z):
        ...
        # mean/background density of the universe at redshift z (function of Hubble parameter & cosmology)
    ...
    def init_mass_function(self,ms):
        # starting from mass of halo, it returns:
        # sigma squared, the variance in the halo profile P(k)^lin
        # n(m,z), the halo mass function
        # bh, bias factor
        self.ms = ms
    ...
    def get_fsigmaz(self):
        # Returns nu f(nu) as the fit function for the halo density in the CDM model 
        sigma2 = self.sigma2
        deltac = self.p['st_deltac']         # critical overdensity today \delta_c(z=0) = 1.686
        if self.mode=="sheth-torman":
            sigma = np.sqrt(sigma2)
            A = self.p['st_A']
            a = self.p['st_a']
            p = self.p['st_p']
            # this is equation (B7) in Matt's paper appendix
    ...
    def get_bh(self):
        ...
        if self.mode=="sheth-torman":
            ...
            a = self.p['st_a']
            p = self.p['st_p']
            # This is equation  B8 in Matt's paper appendix
    ...
    def get_nzm(self):
        # Halo mass function n(m,z) or equation B4 in Matt's paper appendix
        sigma2 = self.sigma2
        ms = self.ms
...
# This is rho_s extracted from eq (76) in the review paper
