...
class Cosmology(object):
    ...
    def CMB_power_spectra(self):
        ...
        return self.results.get_cmb_power_spectra(self.pars, CMB_unit='muK', raw_cl=True)
    ...
    def rho_matter_z(self,z):
        # om0 = omega matter today
        # (1+z)^3 = a^-3 matter scaling with scale factor
...
#     def C_darkp(self,ells,zs,ks,Pconv,gzs,dtaudchi,zmin=None,zmax=None):
#        gzs = np.asarray(gzs)
#        chis = self.comoving_radial_distance(gzs)
#        hzs = self.h_of_z(gzs) # 1/Mpc
#        nznorm = np.trapz(dtaudchi,gzs)
#        Wz1s = dtaudchi/nznorm
#        Wz2s = dtaudchi/nznorm
#
#        return limber_integral(ells,zs,ks,Pconv,gzs,Wz1s,Wz2s,hzs,chis)
